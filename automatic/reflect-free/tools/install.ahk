; Parameters:
;   Path to agent
;   Type of trial install - Home, Workstation, Server, ServerPlus
; 
...
exe_pattern := "reflect*.exe"   
tmp_dir     := TEMP . "\MacriumInstall"

if (!exe_path) {
    loop, Files, %A_ScriptDir%\%exe_pattern%
        exe_path := A_LoopFileFullPath
}

if (!exe_path) {
    Msgbox Can't find Macrium downlodad agent
    return
}
...
ControlSetText, Edit2, %tmp_dir%, ahk_id %hwnd%
